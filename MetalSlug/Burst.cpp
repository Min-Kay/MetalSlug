...
void Burst::Render(HDC _hdc)
{
	...
	if (active)
	{
		...
		if (!spawning)
		{
			...
			if (animIndex < 4)
			{
				GdiTransparentBlt(_hdc,rect.left + x , rect.top + y, 80, 100, drawingDC,animIndex * 40, 1725,40,50,RGB(0,248,0));
			}
			else if (animIndex < 13)
			{
				GdiTransparentBlt(_hdc, rect.left + x, rect.top + y, 100, 330, drawingDC, (animIndex  % 9) * 50, 1775, 50, 165, RGB(0, 248, 0));
			}
			else if (animIndex < 25)
			{
				GdiTransparentBlt(_hdc, rect.left + x, rect.top + y, 100, 330, drawingDC, (animIndex  % 12 ) * 50,1940 , 50, 165, RGB(0, 248, 0));
			}
			else
			{
				GdiTransparentBlt(_hdc, rect.left + x, rect.top + y, 100, 330, drawingDC, (animIndex % 4) * 50, 2105, 50, 165, RGB(0, 248, 0));
			}
		...
		else if(!maintain) 
		{
			...
			GdiTransparentBlt(_hdc, rect.left + x, rect.top + y, 100, 330, drawingDC, animIndex * 50, 2260, 50, 165, RGB(0, 248, 0));
			GdiTransparentBlt(_hdc, rect.left - 16.f + x, rect.bottom  - 60.f + y, 132, 60, drawingDC, animIndex * 66, 2815, 66, 30, RGB(0, 248, 0));
		...
		else 
		{
			...
			if (animIndex < 3)
			{
				GdiTransparentBlt(_hdc, rect.left + x, rect.top + y, 100, 330, drawingDC, animIndex * 50, 2425, 50, 165, RGB(0, 248, 0));
			}
			else
			{
				GdiTransparentBlt(_hdc, rect.left + x, rect.top + y, 100, 330, drawingDC, animIndex * 60, 2590, 60, 165, RGB(0, 248, 0));
			}
